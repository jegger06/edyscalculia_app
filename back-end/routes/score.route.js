...
router.get('/pre-test/:lesson_id', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  db.query(sql, [accountId, lessonId, difficultyId], (err, result) => {
    if (err) {
      return res.json({
        success: false,
...
router.post('/create', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  db.query(sql, [lessonId, difficultyId, accountId, score], (err, result) => {
    if (err) {
      return res.json({
        success: false,
...
router.get('/lists/:difficulty_id', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  db.query(sql, [difficultyId, accountId], (err, result) => {
    if (err) {
      return res.json({
        success: false,
