...
router.post('/create', passport.authenticate('jwt', { session: false }), (req, res) => {
    if (req.user.type_slog === 'admin') {
        db.getConnection((err, connection) => {
            ...
            connection.query(sql, [chapter_slog], (err, result) => {
                if (err) {
                    connection.release();
                    return res.json({
                        success: false,
                ...
                if (!result.length) {
                    ...
                    connection.query(sql, [account_id, chapter_slog, chapter_text, date_created], (err, result) => {
                        connection.release();
                        if (err) {
                            return res.json({
                                success: false,
                ...
                if (!result.length) {
                    ...
                    connection.query(sql, [chapter_slog], (err, result) => {
                        ...
                        if ((result[0] && result[0].chapter_id == id) || !result.length) {
                            ...
                            connection.query(sql, [chapter_slog, chapter_text, chapter_status, date_updated, id], (err, result) => {
                                connection.release();
                                if (err) {
                                    return res.json({
                                        success: false,
